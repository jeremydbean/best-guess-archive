...
**`impossible-clue-counts`** is a warning, not an error: a clue reporting more `correct` than `guesses` is impossible, but two v1 rounds already carry it (SKUNK clue 5 on Apr 8, 2026 and PARADE clue 5 on Feb 17, 2026) and the real figures are unrecoverable. Never "fix" these by inventing a number. New imports should never add one — check the pair before importing.

**`alsoAccepted`** records an alternate wording the show itself counted as correct on the night — a comma-separated string, in the same ALL CAPS style as `secretItem` (e.g. `secretItem: "THE KITCHEN SINK"` with `alsoAccepted: "SINK"`). Omit the field entirely when there is no alternate. Three things read it automatically, so nothing else needs hand-editing: Play mode accepts every listed spelling, the stats page's "Most Popular Wrong Guesses" filters them out (an accepted alternate was a correct answer, so it does not belong in a wrong-guess ranking), and the details screen shows an "Also accepted" line under the answer. That line is spoiler-gated with the `db-explanation` class, since an alternate is usually a near-miss of the answer itself. **Do not use `note` for this** — `note` is reserved for cancelled episodes and renders as a red warning badge.
