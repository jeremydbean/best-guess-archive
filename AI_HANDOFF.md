...
- **Stats page: "Medal Winners Over Time" + "Payout Per Winner Over Time" charts**: Two stacked single-axis log line charts on the Stats page under "Players Over Time" (replaced an earlier single 6-line dual-axis chart that read as too chaotic). Each has 3 color-coded lines — gold `#FBBF24` / silver `#CBD5E1` / bronze `#FB923C` — for **v2** episodes by air date: one chart for winner counts (`#chart-medal-winners`), one for $/winner (`#chart-medal-payout`). Empty tiers (0 winners) map to `null` → rendered as gaps and skipped in tooltips. Data computed in `_getStatsData()` (`v2SortedByDate`, `v2TimeLabels`, `gold/silver/bronzeWinnersData`, `gold/silver/bronzePayoutData`); both built via a shared `buildMedalChart` helper in `_renderStatsCharts()` (`this._charts.medalWinners` / `medalPayout`); nodes `chartMedalWinners`/`chartMedalPayout`. Tooltip title = `secretItem · date`. Reuses `crosshairLinePlugin`, `_compactNumber`, `_logTickFilter`. `npm run audit` passes.
