...
- **Stats page: new "Medal Tiers Over Time" chart**: Added a full-width line chart (`#chart-medal-time`) on the Stats page directly under "Players Over Time", modeled on it. Shows, for each **v2** episode by air date: gold/silver/bronze **winner counts** (solid lines, left log axis) and **amount won per winner** (dashed lines, right log axis) — 6 series, colors gold `#FBBF24` / silver `#CBD5E1` / bronze `#FB923C`. Empty tiers (0 winners) map to `null` so they render as gaps and tooltips skip them (`tooltip.filter`). Data is computed in `_getStatsData()` (`v2SortedByDate`, `v2TimeLabels`, `gold/silver/bronzeWinnersData`, `gold/silver/bronzePayoutData`) and the chart is built in `_renderStatsCharts()` (`this._charts.medalTime`); node registered as `nodes.chartMedalTime`. Tooltip title shows `secretItem · date`. Reuses the existing `crosshairLinePlugin`, `_compactNumber`, and `_logTickFilter` helpers. `npm run audit` passes (inline scripts parse).
