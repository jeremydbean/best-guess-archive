...
- **⚠️ June 17, 2026 episode imported as RESULTS PENDING — BRAIN (R1) and HIDE AND SEEK (R2), host Hunter March** (both `format: "v2"`). Only clue text, answers, host (inferred from the "fun with Hunter" clue + on-screen host), and popular wrong guesses were available from screenshots — **no winner counts, guess counts, or payouts**. So both games are stored with `goldWinners/silverWinners/bronzeWinners: 0`, `totalWinners: 0`, all payouts `0`, **no medal clue fields**, and an `adminNote` flagging results pending. The transcript is a structural skeleton (clue text present in Results sections to satisfy the audit; dialogue + counts pending). **TODO to backfill when numbers arrive**: per-clue correct/guesses, gold/silver/bronze winners + payouts, medal clue assignments (Hide and Seek known from screenshot: gold Clue 1 / silver Clue 2 / bronze Clue 3), winner names, and the real transcript. Round order is from the end-of-episode summary screen (Puzzle 1 = BRAIN, Puzzle 2 = HIDE AND SEEK). Wrong guesses: R1 `CHERRY, GOOGLE, ELON MUSK, SIRI, APPLE`; R2 `ROCKET, TIMER, TAG, ELEVATOR, FIREWORKS`. `npm run audit` passes 0/0.
